...
--         
    ...
    awful.key({ modkey, "Shift"   }, "n"     , function() 
                                                  ...
                                                  local t = awful.tag.add(c.class,{screen= (client.focus and client.focus.screen or mouse.screen),layout=awful.layout.suit.tile })
...
--     awful.key({ modkey, "Mod1"    }, "Down",  function () indicator.resize("down")  end),
   ...
   awful.key({ modkey, }, "Up",  awful.tag.history.restore),
    ...
    awful.key({                   }, "#164"  , function () utils.mouseManager.switchTo(1)       end ),
    awful.key({                   }, "#166"  , function () utils.mouseManager.switchTo(2)       end ),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ "Mod1" }, "l", function() awful.util.spawn("sflock -c ' '") end),
...
run_once("fcitx -r --enable fcitx-qimpanel")
run_once("sogou-autostart")
run_once("dropbox")
run_once("compton -b")
run_once("easystroke")
run_once("setxkbmap -option 'caps:ctrl_modifier'")
run_once("xcape -e 'Caps_Lock=Escape;Control_L=Escape;Alt_R=End;Control_R=Home'")

mygmail = wibox.widget.textbox()
gmail_t = awful.tooltip({ objects = { mygmail },})
mygmailimg = wibox.widget.imagebox(beautiful.mail)
