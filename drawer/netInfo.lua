...
--util.spawn("/bin/bash -c 'while true;do "..util.getdir("config") .."/Scripts/connectedHost2.sh && sleep 5;done'")
--util.spawn("/bin/bash -c 'while true;do "..util.getdir("config") .."/Scripts/localNetLookup2.sh > /tmp/localNetLookup && sleep 5;done'")

local function update()
    local connectionInfo
    util.spawn("/bin/bash -c '"..util.getdir("config") .."/Scripts/connectedHost2.sh'")
    ...
    local localValue = ""
    util.spawn("/bin/bash -c '"..util.getdir("config") .."/Scripts/localNetLookup2.sh > /tmp/localNetLookup'")
    ...
    for i=0 , #(data.connectionInfo or {}) do
        if data.connectionInfo[i] then
            ...
            application.fit = function()
                return 128,connMenu.item_height
            end
...
local function repaint(margin)
    ...
    vicious.register                 (netUpGraph, vicious.widgets.net  , '${eno1 up_kb}'  ,1)
    setup_graph(netDownGraph)
    vicious.register                 (netDownGraph, vicious.widgets.net, '${eno1 down_kb}',1)
    ...
    mainMenu = menu({width=300,arrow_type=radical.base.arrow_type.CENTERED})
    mainMenu:add_widget(radical.widgets.header(mainMenu,"GRAPH"),{height = 20 , width = 300})
    mainMenu:add_widget(mar ,{height = 73, width = 300})
    mainMenu:add_widget(radical.widgets.header(mainMenu,"IP"),{height = 20 , width = 300})
    mainMenu:add_widget(ipm       ,{height = 50, width = 300})

    local imb = wibox.widget.imagebox()
    imb:set_image(beautiful.path .. "Icon/reload.png")
    mainMenu:add_widget(radical.widgets.header(mainMenu,"CONNECTIONS",{suffix_widget=imb}),{height = 20 , width = 300})

    if data.connectionInfo ~= nil then
        connMenu = embed({width=298,max_items=5,has_decoration=false,has_side_deco=true})
        mainMenu:add_embeded_menu(connMenu)
    end
    mainMenu:add_widget(radical.widgets.header(mainMenu,"PROTOCOLS",{suffix_widget=imb}),{height = 20 , width = 300})

    protMenu = radical.widgets.piechart()
    mainMenu:add_widget(protMenu,{height = 100 , width = 100})
    protMenu:set_data(protocolStat)

    mainMenu:add_widget(radical.widgets.header(mainMenu,"APPLICATIONS",{suffix_widget=imb}),{height = 20 , width = 300})

    appMenu = embed({width=298,max_items=3,has_decoration=false,has_side_deco=true})
...
local function new(margin, args)
    ...
    vicious.register(volumewidget2  , vicious.widgets.net   ,  '${eno1 up_kb}'   ,3 )
    vicious.register(volumewidget3, vicious.widgets.net   ,  '${eno1 down_kb}' ,3 )
